...
export class MessageCache {
  ...
  buildContextBlock(roomId, agentId, options = {}) {
    ...
    const messages = unread.slice(-maxMessages);

    const lines = [`[teamroom-context room=${roomId} unread=${unread.length}]`];
    for (const msg of messages) {
      const time = new Date(msg.ts).toISOString().slice(11, 16);
