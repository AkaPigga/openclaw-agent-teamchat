...
function resolveAutopilotTargets(room, mentionedAgents, sourceAgent) {
  // Phase 3+: mention-driven dispatch.
  // If there are mentions, only dispatch the mentioned agents (精准触发).
  // If no mentions, fall back to trigger-based dispatch (mentions-or-all only).
  if (mentionedAgents.length > 0) {
    if (!room.autopilot.mentionedDispatch) return [];
    // Only dispatch agents that are both mentioned AND in memberAgents
    let targets = mentionedAgents.filter((a) => room.memberAgents.includes(a));
    if (!room.includeSourceAgent && sourceAgent) {
  ...
  let targets = [];
  if (room.autopilot.trigger === "mentions-or-all") {
    targets = room.memberAgents;
  } else {
    // mentions-only trigger: no mentions → no autopilot
    return [];
  }
  if (!room.includeSourceAgent && sourceAgent) {
    targets = targets.filter((agentId) => agentId !== sourceAgent);
...
async function runAutopilotDispatch(api, room, agentId, accountId, prompt, sessionKey) {
...
export function createTeamChatRuntime(api, pluginConfig, state) {
  ...
  function beforeToolCall(event, ctx) {
    ...
    // ─── autoRelay: handled in messageSending after message is written to cache ───
  ...
  async function messageSending(event, ctx) {
    ...
    // ─── mention-driven autopilot: agent @mentions another agent ───
    // Skip if autoRelay is enabled (autoRelay handles this via beforeToolCall)
    if (room.autopilot.enabled && room.autopilot.mentionedDispatch && !room.autoRelay?.enabled && mentionedAgents.length > 0) {
      const targets = mentionedAgents.filter(
        (a) => room.memberAgents.includes(a) && a !== sourceAgent
      );
      for (const targetAgent of targets) {
        const accountIdForTarget = room.resolveAccountForAgent(targetAgent);
        const { sessionKey: agentSessionKey } = resolveAgentSessionKey(room, targetAgent);
        if (!agentSessionKey) continue;
        if (room.autopilot.dryRun) {
          api.logger.info(`[teamchat] autopilot dry-run (agent mention): ${sourceAgent} → ${targetAgent}`);
          continue;
        }
        runAutopilotDispatch(api, room, targetAgent, accountIdForTarget, content, agentSessionKey).catch(
          (err) => api.logger.error(`[teamchat] autopilot agent-mention dispatch error: ${err?.message}`)
        );
      }
    }
  ...
  async function messageReceived(event, ctx) {
    ...
    // Cache id must never be a provider-native id (om_xxx) — use a stable composite key
    const cacheMessageId = `msg:${roomId}:${timestamp}:${textHash(content)}`;
    const senderLabel =
    ...
    for (const signal of taskSignals) {
      ...
      // ─── v3: Sync task signal to TaskBoard (slots mode) ───
      ...
      if (!existing && (normalizedStatus === "create" || normalizedStatus === "ack")) {
        ...
        // Also write the actor's slot via updateTask so slot is populated
        if (actor !== "external") {
        ...
        // For all other statuses (or if task already exists), write into actor's slot
        taskBoard.updateTask(room.id, {
          taskId: signal.taskId,
          status: normalizedStatus,
          actor,
    ...
    // ─── v2: Non-autopilot agents get cache context injected via enqueue ───
    if (!echoedAgent) {
      ...
      if (pluginConfig.teamroom.stickyOutput.enabled) {
        ...
        const stickyTargets =
          seedMode === "all-members-on-external" ? room.memberAgents : forwardTargets;
      ...
      for (const targetAgent of forwardTargets) {
        if (autopilotTargetSet.has(targetAgent)) continue;
        // Context injection happens in beforeAgentStart via prependContext.
        // No markAllAsRead here — watermark advances when agent actually reads context.
      }
    }
    ...
    if (autopilotTargets.length > 0) {
      ...
      for (const targetAgent of autopilotTargets) {
        ...
        const cacheContext = messageCache.buildContextBlock(room.id, targetAgent, { maxMessages: pluginConfig.teamroom.contextMaxMessages, maxChars: pluginConfig.teamroom.contextMaxChars });
        ...
        const { sessionKey: agentSessionKey } = resolveAgentSessionKey(room, targetAgent);
        const result = await runAutopilotDispatch(api, room, targetAgent, accountId, prompt, agentSessionKey);
        records.push({
          roomId: room.id,
          agentId: targetAgent,
          ok: result.ok,
        ...
        if (result.ok && result.replyText) {
          const normalizedReply = asString(result.replyText);
          if (normalizedReply) {
  ...
  function beforeMessageWrite(event, ctx) {
    ...
    api.logger.debug?.(
      `[teamchat] before_message_write: room=${room.id} agent=${agentId} session=${asString(ctx?.sessionKey) || "unknown"}`
    );
  }
  ...
  function commandHandler(ctx) {
    ...
    if (verb === "reset-watermark") {
      const selectedRooms = roomId && roomId !== "all" && pluginConfig.teamroom.roomsById[roomId]
        ? [pluginConfig.teamroom.roomsById[roomId]]
        : pluginConfig.teamroom.rooms;
      const now = Date.now();
      for (const room of selectedRooms) {
        for (const agentId of room.memberAgents) {
          messageCache.markAsRead(room.id, agentId, "", now);
        }
      }
      return { text: `[teamchat] watermark reset to now for ${selectedRooms.length} room(s).` };
    }

    return {
      text: `[teamchat] unknown subcommand.\nUse /${commandName} status | /${commandName} tasks [roomId|all] | /${commandName} task create <taskId> [owner=<agentId>] [note=<text>] [room=<roomId>] | /${commandName} task update <taskId> <status> [owner=<agentId>] [note=<text>] [room=<roomId>] | /${commandName} reset <roomId|all> | /${commandName} reset-watermark [roomId|all]`
    };
  }
  ...
  function beforeAgentStart(event, ctx) {
    ...
    // Safety check: block heartbeat/main sessions (agent:xxx:main format)
    const sessionKey = asString(ctx?.sessionKey);
    if (sessionKey && sessionKey.startsWith("agent:") && !sessionKey.includes(`:${roomId}`)) {
      api.logger.warn(
        `[teamchat] blocked unsafe relay route: room=${room.id} agent=${agentId} session=${sessionKey}`
      );
      return;
    }

    const cacheCtx = messageCache.buildContextBlock(room.id, agentId, {
      maxMessages: pluginConfig.teamroom.contextMaxMessages,
      maxChars: pluginConfig.teamroom.contextMaxChars
    });
    if (!cacheCtx) return;

    const boardCtx = taskBoard.buildBoardContext(room.id, agentId);
    const protocolGuide = room.protocol.enabled ? buildProtocolGuide(room) : "";
    ...
    messageCache.markAllAsRead(room.id, agentId);
